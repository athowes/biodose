...
# (note: will break if e.g. there are up to 6 dicentrics in a cell)
...
npboot <- function(observed, design, dataset) {
  ...
  subset <- dataset$d %in% d # sum(subset) == length(d)?
  measurement <- dataset[subset,]
  measurement[ ,"m"] <- m
  s <- resample5(measurement)$s
  ...
  est <- (-h1 + sqrt(h1^2 - 4*h2*(h0 - s/m)))/(2*h2) # Dose estimate
  return(est)
}
